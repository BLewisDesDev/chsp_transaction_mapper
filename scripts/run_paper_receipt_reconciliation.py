#!/usr/bin/env python3
"""
Paper Receipt Reconciliation Runner

Processes paper receipt transactions using the file path configured in caura_config.yaml
under data.paper_receipts_file_path. No command line arguments required.
"""
...
def main():
    ...
    # Get paper receipts CSV file path from config
    csv_file = config.get('data', {}).get('paper_receipts_file_path')
    if not csv_file:
        print("Error: paper_receipts_file_path not found in config")
        sys.exit(1)

    # Validate file exists
    if not Path(csv_file).exists():
        print(f"Error: Paper receipts CSV file not found: {csv_file}")
        sys.exit(1)

    print(f"Processing paper receipt transactions from: {csv_file}")
    ...
    try:
        ...
        # Run Paper Receipt import
        importer = PaperReceiptImporter(config, matcher)
        
        # Extract transactions for Excel export
        print("Extracting transactions for Excel export...")
        transactions = importer.extract_transactions(csv_file)
        ...
        excel_exporter = StripeInterReport(client_map)  # Can reuse for paper receipts
        output_dir = Path(config['paths']['output_base']) / config['paths']['reports_subdir']
        excel_file = output_dir / f"{report.run_id}_paper_receipt_reconciliation.xlsx"
        ...
        # Print summary
        print(f"\n🎯 Paper Receipt Reconciliation Complete")
        ...
        # Print suburb verification stats if available
        suburb_verified = sum(1 for mr in report.match_results 
                            if mr.match_method == "paper_receipt_name_fuzzy" 
                            and mr.match_details.get("suburb_verified", False))
        if suburb_verified > 0:
            print(f"\n🏘️  Suburb Verification:")
            print(f"   Matches with suburb verification: {suburb_verified}")

    except Exception as e:
        print(f"❌ Paper receipt reconciliation failed: {e}")
        sys.exit(1)
